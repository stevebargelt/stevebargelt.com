...
I knwo it's no the Macintosh HD so it must be boot. Let's add a file to the root of the SD card 'boot' that will tell our device how to find and connect to our WiFi network.
...
Once again I'll refer you to  [on raspberrypi.org](https://www.raspberrypi.org/documentation/remote-access/ip-address.md) for more detail and more options. I'll quickly run through what I do:
...
Your local network is most likely in the 192.168.1.x subnet or the 10.0.0.x subnet. So depending o your local network you can run
...
The default password for the pi uer is raspberry.

And connected!

## Change the pi user's default password

Now that we can see our new Raspberry Pi on the network ans connect to it ove SSH, we will and to change a few settings. First up, lets change that default password for the pi user.
